...
test("limite de fotos: 3 en Basico, 8 en Premium", () => {
  assert.equal(limiteFotos({ plan: "basico" }), 3);
  assert.equal(limiteFotos({ plan: "premium", premium_hasta: null }), 8);
  assert.equal(LIMITE_FOTOS.basico, 3);
  assert.equal(LIMITE_FOTOS.premium, 8);
  // Un Premium vencido vuelve al limite del gratis, igual que el resto.
  assert.equal(limiteFotos({ plan: "premium", premium_hasta: vencimientoEnDias(-1) }), 3);
  assert.equal(limiteFotos({ plan: "premium", premium_hasta: vencimientoEnDias(10) }), 8);
});
